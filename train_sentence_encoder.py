...
flags.DEFINE_integer('print_every',    1,    'how often to print current loss')
...
def main(_):
    ...
    word_tensors, label_tensors, max_doc_length, max_line_length, word_vocab = load_data(
        FLAGS.data_dir,
        FLAGS.master_file,
        FLAGS.max_doc_length,
        FLAGS.max_sen_length
    )

    FLAGS.max_sen_length = max_line_length

    train_reader = DOMReader(word_tensors, label_tensors, FLAGS.batch_size)

    valid_reader = DOMReader(word_tensors, label_tensors, FLAGS.batch_size)
