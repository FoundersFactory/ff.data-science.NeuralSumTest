...
def cnn_sen_enc(char_vocab_size,
                word_embed_size=50,
                batch_size=20,
    ...
    input_ = tf.placeholder(tf.int32, shape=[batch_size, max_doc_length, max_line_length], name='input')

    """ First, embed words to sentence """
    with tf.variable_scope('Embedding'):
        if pretrained is not None:
            word_embedding = tf.get_variable(name='word_embedding', shape=[char_vocab_size, word_embed_size],
        ...
        input_embedded = tf.reshape(input_embedded, [-1, max_line_length, word_embed_size])
...
def self_prediction(outputs, char_vocab_size):
    ...
    with tf.variable_scope('SelfPrediction') as scope:
        for idx, output in enumerate(outputs):
            ...
            logits_pretrain.append(linear(output, char_vocab_size))
...
def loss_pretrain(logits, batch_size, max_doc_length, char_vocab_size):
    # reconstruction loss
    with tf.variable_scope('Loss'):
        targets = tf.placeholder(tf.float32, [batch_size, max_doc_length, char_vocab_size], name='targets')
        target_list = [tf.squeeze(x, [1]) for x in tf.split(targets, max_doc_length, 1)]
...
if __name__ == '__main__':
    with tf.Session() as sess:
        ...
        with tf.variable_scope('Model'):
            graph = cnn_sen_enc(char_vocab_size=100)
        ...
        with tf.variable_scope('Model', reuse=True):
            tgraph = cnn_sen_enc(char_vocab_size=100)
        ...
        with tf.variable_scope('Model2'):
            graph2 = cnn_sen_enc(char_vocab_size=100)
        ...
        with tf.variable_scope('Model3'):
            pgraph = cnn_sen_enc(char_vocab_size=100)
            pgraph.update(bilstm_doc_enc(pgraph.input_cnn, dropout=0.5))
            pgraph.update(self_prediction(pgraph.enc_outputs, char_vocab_size=100))
            pgraph.update(loss_pretrain(pgraph.plogits, batch_size=20, max_doc_length=35, char_vocab_size=100))
